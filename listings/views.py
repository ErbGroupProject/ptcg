...
def create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            listing.user_name = Profile.objects.get(
                user=request.user
            )
            listing.save()
...
def my_listings(request):
    ...
    completed_listings = (
        listings
        .filter(chat_room__trade_finished=True)
        .distinct()
    )
...
def index(request):
    ...
    qs = (
        ...
        .annotate(
            completed_at=Max(
                ...
                filter=Q(
                    chat_room__trade_finished=True
                ),
            ),
        )
        .annotate(
    ...
    if q:
        qs = qs.filter(
            ...
            | Q(
                user_name__user__username__icontains=q
            )
        )
    ...
    series = request.GET.get(
        "series",
        "",
    ).strip()

    place = request.GET.get(
        "place",
        "",
    ).strip()
...
def card_listings(request):
    ...
    generations = Generation.objects.order_by(
        "name"
    )
...
def detail(request, listing_id):
    ...
    listing = get_object_or_404(
        Tradelist.objects.select_related(
            "user_name__user"
        ),
        id=listing_id,
    )
